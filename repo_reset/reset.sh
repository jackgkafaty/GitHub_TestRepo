...
# Create pull request
gh pr create --title "$new_branch" --body $'#### What was done?\ncopilot:summary\n#### Why was this change done?\nTo measure the time it takes to execute a database query, which is used later to check for potential NoSQL denial-of-service (DoS) attacks\n#### How was this done?\ncopilot:walkthrough' --base main --head $new_branch
